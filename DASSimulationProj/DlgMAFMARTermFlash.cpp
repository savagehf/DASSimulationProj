// DlgMAFMARTermFlash.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CDlgMAFMARTermFlash, CDialog)

CDlgMAFMARTermFlash::CDlgMAFMARTermFlash(CWnd* pParent /*=NULL*/)
	: CDialog(CDlgMAFMARTermFlash::IDD, pParent)
{
	m_bStep1 = FALSE;
	m_bStep2 = FALSE;
	m_bStep1Flash = FALSE;
...
CDlgMAFMARTermFlash::~CDlgMAFMARTermFlash()
{
}

void CDlgMAFMARTermFlash::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(CDlgMAFMARTermFlash, CDialog)
	ON_WM_PAINT()
	ON_WM_TIMER()
	ON_WM_ERASEBKGND()
	ON_WM_CLOSE()
	ON_BN_CLICKED(IDC_BTN_PAUSE_MAFMAR_FLASH, OnBnClickedBtnPauseMafMarFlash)
END_MESSAGE_MAP()



BOOL CDlgMAFMARTermFlash::OnInitDialog()
{
...
BOOL CDlgMAFMARTermFlash::OnEraseBkgnd(CDC* pDC )
{
	return TRUE;
}

void CDlgMAFMARTermFlash::OnClose()
{
	...
	if (NULL != pMain)
	{
		...
		pView->StartDrawMAFMARPhase1AnswerSignal(TRUE);
	}

	__super::OnClose();
}
void CDlgMAFMARTermFlash::OnPaint()
{
	if (IsIconic())
	{
...
void CDlgMAFMARTermFlash::DrawTitleTips(CDC* pDC, CRect rcClient)
{
	...
	pDC->DrawText(_T("双向服务规划阶段(一)"), &rcTitle, DT_CENTER);

	pDC->SelectObject(pOldPen);
}

void CDlgMAFMARTermFlash::DrawClient(CDC* pDC)
{
	...
	if (m_bStep2)
	{
		...
		CString strText("2.DASCON检验用户所请求的双向服务是否有效，返回服务请求响应。");
...
void CDlgMAFMARTermFlash::DrawDASHLine(CDC* pDC, CPoint ptStart, CPoint ptEnd)
{
...
void CDlgMAFMARTermFlash::DrawMultiLine(CDC* pDC, CRect rect, CString strText)
{
...
void CDlgMAFMARTermFlash::OnTimer(UINT_PTR nIDEvent)
{
...
int CDlgMAFMARTermFlash::DrawArrowLine(HDC hDC, LPPOINT lpPointStart, LPPOINT lpPointEnd, 
...
void CDlgMAFMARTermFlash::OnBnClickedBtnPauseMafMarFlash()
{

	if (m_bPause)
	{
		m_bPause = FALSE;
		GetDlgItem(IDC_BTN_PAUSE_MAFMAR_FLASH)->SetWindowText(_T("暂定"));
		SetTimer(enmTimer_Step1, 500, NULL);
	}
	else
	{
		m_bPause = TRUE;
		GetDlgItem(IDC_BTN_PAUSE_MAFMAR_FLASH)->SetWindowText(_T("开始"));
		m_bStep1 = FALSE;
		m_bStep2 = FALSE;
		m_bStep1Flash = FALSE;
		m_bStep2Flash = FALSE;
