// DlgMAFPhase2.cpp : implementation file
//
...
IMPLEMENT_DYNAMIC(CDlgMAFPhase2, CDialog)

CDlgMAFPhase2::CDlgMAFPhase2(CWnd* pParent /*=NULL*/)
	: CDialog(CDlgMAFPhase2::IDD, pParent)
{
...
CDlgMAFPhase2::~CDlgMAFPhase2()
{
}

void CDlgMAFPhase2::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(CDlgMAFPhase2, CDialog)
	ON_WM_PAINT()
	ON_WM_TIMER()
	ON_WM_ERASEBKGND()
	ON_WM_CLOSE()
	ON_BN_CLICKED(IDC_BTN_MAF2_PAUSE_FLASH, &CDlgMAFPhase2::OnBnClickedBtnMaf2PauseFlash)
END_MESSAGE_MAP()


// CDlgMAFPhase2 message handlers
BOOL CDlgMAFPhase2::OnInitDialog()
{
...
void CDlgMAFPhase2::DrawTitleTips(CDC* pDC, CRect rcClient)
{
	...
	pDC->DrawText(_T("DAF服务请求阶段(二)"), &rcTitle, DT_CENTER);

	pDC->SelectObject(pOldPen);
}


BOOL CDlgMAFPhase2::OnEraseBkgnd(CDC* pDC )
{
	return TRUE;
}

void CDlgMAFPhase2::OnClose()
{
...
void CDlgMAFPhase2::OnPaint()
{
	...
	else
	{
		...
		GetClientRect(rect);
		CBitmap bmp;
		if (bmp.LoadBitmap(IDB_MAF_PHASE_2_BITMAP))
		{
			BITMAP bmpInfo;
...
void CDlgMAFPhase2::DrawClient(CDC* pDC)
{
	CRect rect;
	GetClientRect(rect);
	...
	DrawTitleTips(pDC, rect);
 	...
 	if (m_bStep1)
 	{
 		//绘制提示信息
 		CRect rcTip1;
 		rcTip1.top = m_rcNCC.top-10;
 		rcTip1.bottom = m_rcNCC.bottom;
 		rcTip1.left = m_rcNCC.right;
 		rcTip1.right = m_rcPOCC.left-2;
		pDC->Rectangle(&rcTip1);
 		CString strText("1.用户POCC通过DAS规划工具向DASCON发起DAS服务请求(Service Request)。");
 	...
 	if (m_bStep2)
 	{
		...
		pDC->Rectangle(&rcTip2);
 		...
 		CPoint ptDashStart(rcTip2.right, rcDASCON.top-15);
 		CPoint ptDashEnd(rcDASCON.right+(rcIONET.left-rcDASCON.right)/2, rcDASCON.top-15);
 		DrawDASHLine(pDC, ptDashStart, ptDashEnd);
 		ptDashStart.SetPoint(rcDASCON.right+(rcIONET.left-rcDASCON.right)/2, rcDASCON.top-15);
 		ptDashEnd.SetPoint(rcDASCON.right+(rcIONET.left-rcDASCON.right)/2, rcDASCON.top+rcDASCON.Height()/2);
 		DrawDASHLine(pDC, ptDashStart, ptDashEnd);*/
		int abc;
		//返回线条
		//dascon-->ionet
		...
		CRect rcTip4;
		rcTip4.top = m_rcIONET.bottom+10;
		rcTip4.bottom = m_rcPOCC.bottom-10;
		rcTip4.left = m_rcPOCC.right+2;
		rcTip4.right = m_rcIONET.right+10;
		pDC->Rectangle(&rcTip4);

		strText = "3.用户POCC接收请求响应。";
 	...
 	if (m_bStep3)
 	{
 		...
 		CString strText("3.用户POCC接收请求响应。");
 		DrawMultiLine(pDC, rcTip4, strText);*/
...
void CDlgMAFPhase2::DrawDASHLine(CDC* pDC, CPoint ptStart, CPoint ptEnd)
{
...
void CDlgMAFPhase2::DrawMultiLine(CDC* pDC, CRect rect, CString strText)
{
...
void CDlgMAFPhase2::OnTimer(UINT_PTR nIDEvent)
{
...
int CDlgMAFPhase2::DrawArrowLine(HDC hDC, LPPOINT lpPointStart, LPPOINT lpPointEnd, 
...
void CDlgMAFPhase2::OnBnClickedBtnMaf2PauseFlash()
{
	if (m_bPause)
	{
		m_bPause = FALSE;
		GetDlgItem(IDC_BTN_MAF2_PAUSE_FLASH)->SetWindowText(_T("暂定"));
		SetTimer(enmTimer_Step1, 500, NULL);
	}
	else
	{
		m_bPause = TRUE;
		GetDlgItem(IDC_BTN_MAF2_PAUSE_FLASH)->SetWindowText(_T("开始"));
		m_bStep1 = FALSE;
		m_bStep2 = FALSE;
		m_bStep3 = FALSE;
		m_bStep1Flash = FALSE;
