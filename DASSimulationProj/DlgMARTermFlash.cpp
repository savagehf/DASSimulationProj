// DlgMARTerminationFlash.cpp : implementation file
//
...
// CDlgMARTermFlash dialog

IMPLEMENT_DYNAMIC(CDlgMARTermFlash, CDialog)


static int s_step = 40;

CDlgMARTermFlash::CDlgMARTermFlash(CWnd* pParent /*=NULL*/)
	: CDialog(CDlgMARTermFlash::IDD, pParent)
{
	m_bStep1 = FALSE;
	m_bStep2 = FALSE;
	m_bStep1Flash = FALSE;
...
CDlgMARTermFlash::~CDlgMARTermFlash()
{
}

void CDlgMARTermFlash::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}


BEGIN_MESSAGE_MAP(CDlgMARTermFlash, CDialog)
	ON_WM_PAINT()
	ON_WM_TIMER()
	ON_WM_ERASEBKGND()
	ON_WM_CLOSE()
	ON_BN_CLICKED(IDC_BTN_PAUSE_MAR_FLASH, &CDlgMARTermFlash::OnBnClickedBtnPauseMarFlash)
END_MESSAGE_MAP()


BOOL CDlgMARTermFlash::OnInitDialog()
{
...
BOOL CDlgMARTermFlash::OnEraseBkgnd(CDC* pDC )
{
	return TRUE;
}

void CDlgMARTermFlash::OnClose()
{
	...
	if (NULL != pMain)
	{
		...
		pView->StartDrawPhase1AnswerSignal(FALSE);
	}

	__super::OnClose();
}

void CDlgMARTermFlash::OnPaint()
{
	if (IsIconic())
	{
...
void CDlgMARTermFlash::DrawTitleTips(CDC* pDC, CRect rcClient)
{
...
void CDlgMARTermFlash::DrawClient(CDC* pDC)
{
...
// 	{
// 		CRect rcTip1;
// 		rcTip1.left = rcDASCON.left-100;
// 		rcTip1.right = rcTip1.left+300;
// 		rcTip1.top = rcDASCON.bottom+20;
// 		rcTip1.bottom = rcTip1.top+60;
// 
...
// 		//绘制提示信息
// 		CRect rcTip1;
// 		rcTip1.left = rcDASCON.left-10;
// 		rcTip1.right = rcDASCON.right+100;
// 		rcTip1.bottom = rcDASCON.top-40;
// 		rcTip1.top = rcTip1.bottom-80;
// 
...
// 	pDC->SelectObject(pOldFont);
	...
	if (m_bStep2)
	{
		...
		CString strText("2.DASCON检验用户所请求的DAR服务是否有效，返回服务请求响应。");
		DrawMultiLine(pDC, rcTip1, strText);
...
void CDlgMARTermFlash::DrawDASHLine(CDC* pDC, CPoint ptStart, CPoint ptEnd)
{
...
void CDlgMARTermFlash::DrawMultiLine(CDC* pDC, CRect rect, CString strText)
{
...
void CDlgMARTermFlash::OnTimer(UINT_PTR nIDEvent)
{
...
int CDlgMARTermFlash::DrawArrowLine(HDC hDC, LPPOINT lpPointStart, LPPOINT lpPointEnd, 
...
void CDlgMARTermFlash::OnBnClickedBtnPauseMarFlash()
{
	if (m_bPause)
	{
		m_bPause = FALSE;
		GetDlgItem(IDC_BTN_PAUSE_MAR_FLASH)->SetWindowText(_T("暂定"));
		SetTimer(enmTimer_Step1, 500, NULL);
	}
	else
	{
		m_bPause = TRUE;
		GetDlgItem(IDC_BTN_PAUSE_MAR_FLASH)->SetWindowText(_T("开始"));
		m_bStep1 = FALSE;
		m_bStep2 = FALSE;
		m_bStep1Flash = FALSE;
		m_bStep2Flash = FALSE;
