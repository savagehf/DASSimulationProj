	...
	while (ftell (l_file) < filelength (fileno (l_file))) //Loop to scan the whole file 
	{
		...
		switch (l_chunk_id)
        {
			...
			case 0xA300: //// This holds the file name of the texture
			{
				...
				strcpy(matInfo.strFile, strFile);
				break;
			}
...
bool C3DS::Init(char *sFileName)
{
	...
	for(int i = 0;i<m_Model.vMaterials.size();i++)
	{
		...
		{
			CString strFile(_T("ModelData\\"));
			strFile += m_Model.vMaterials[i].strFile;
			m_Model.vMaterials[i].cTxt.LoadBMP(strFile.GetBuffer(0));
		}
	}
	return true;
}
