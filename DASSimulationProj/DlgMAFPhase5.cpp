...
void CDlgMAFPhase5::DrawTitleTips(CDC* pDC, CRect rcClient)
{
	...
	pDC->DrawText(_T("DAF用户数据传输阶段(五)"), &rcTitle, DT_CENTER);
	...
	DrawTitleTips(pDC, rect);
 	...
 	if (m_bStep1)
 	{
		...
		pDC->Rectangle(&rcTip1);
 		...
 		CString strText("1.POCC通过IONet将数据发送到PTP。");
		...
		strText = "2.PTP负责对数据进行格式化路由,并将其发送到DAF前向服务链设备。";
 	...
 	if (m_bStep2)
 	{
 		...
 		//CString strText("2.PTP负责对数据进行格式化路由,并将其发送到MAF前向服务链设备。");
 		//DrawMultiLine(pDC, rcTip2, strText);
		...
		CString strText("3.原有DAF设备负责将用户数据通过RF链路发送到TDRS。");
 	...
 	if (m_bStep3)
 	{
 		...
 		//CString strText("3.原有MAF设备负责将用户数据通过RF链路发送到TDRS。");
 		//DrawMultiLine(pDC, rcTip3, strText);
