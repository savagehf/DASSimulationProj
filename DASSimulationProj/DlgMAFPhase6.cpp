// DlgMAFPhase6.cpp : implementation file
//
...
// CDlgMAFPhase6 dialog

IMPLEMENT_DYNAMIC(CDlgMAFPhase6, CDialog)

CDlgMAFPhase6::CDlgMAFPhase6(CWnd* pParent /*=NULL*/)
	: CDialog(CDlgMAFPhase6::IDD, pParent)
{
	m_bStep1 = FALSE;
	m_bStep2 = FALSE;
	m_bStep3 = FALSE;
...
CDlgMAFPhase6::~CDlgMAFPhase6()
{
}

void CDlgMAFPhase6::DoDataExchange(CDataExchange* pDX)
{
	CDialog::DoDataExchange(pDX);
}

BEGIN_MESSAGE_MAP(CDlgMAFPhase6, CDialog)
	ON_WM_CLOSE()
	ON_WM_PAINT()
	ON_WM_TIMER()
	ON_WM_ERASEBKGND()
	ON_BN_CLICKED(IDC_BTN_MAF6_PAUSE_FLASH, &CDlgMAFPhase6::OnBnClickedBtnMaf6PauseFlash)
END_MESSAGE_MAP()


// CDlgMAFPhase6 message handlers
BOOL CDlgMAFPhase6::OnInitDialog()
{
...
BOOL CDlgMAFPhase6::OnEraseBkgnd(CDC* pDC )
{
	return TRUE;
}

void CDlgMAFPhase6::OnClose()
{
...
void CDlgMAFPhase6::DrawTitleTips(CDC* pDC, CRect rcClient)
{
	...
	pDC->DrawText(_T("DAF服务终止阶段(六)"), &rcTitle, DT_CENTER);

	pDC->SelectObject(pOldPen);
}


void CDlgMAFPhase6::OnPaint()
{
	if (IsIconic())
	{
...
void CDlgMAFPhase6::DrawClient(CDC* pDC)
{
	...
	DrawTitleTips(pDC, rect);
 	...
 	if (m_bStep1)
 	{
		...
		pDC->Rectangle(&rcTip1);
 		...
 		CString strText("1.DASCON发送服务终止命令到DSER,同时通报给NCC");
 	...
 	if (m_bStep2)
 	{
		...
		pDC->Rectangle(&rcTip2);
 		...
 		CString strText("2.PTP停止数据格式化和路由,并且中断到用户POCC的通信链路。");
 		DrawMultiLine(pDC, rcTip2, strText);
 
		//dascon-->dser
		CPoint ptStart(m_rcDSER.left+20, m_rcDASCON.top);
		CPoint ptEnd(m_rcDSER.left+20, m_rcDSER.bottom);
		DrawArrowLine(pDC->GetSafeHdc(), &ptStart, &ptEnd, 8, 10, RGB(255,0,255));
 	...
 	if (m_bStep3)
 	{
		...
		CString strText("3.NCC接收到服务终止命令,停止配置DAF资源。");
 	...
 	if (m_bStep4)
 	{
		...
		CString strText("4.DAF设备接收到服务终止命令,停止工作。");
...
void CDlgMAFPhase6::DrawDASHLine(CDC* pDC, CPoint ptStart, CPoint ptEnd)
{
...
void CDlgMAFPhase6::DrawMultiLine(CDC* pDC, CRect rect, CString strText)
{
...
void CDlgMAFPhase6::OnTimer(UINT_PTR nIDEvent)
{
...
void CDlgMAFPhase6::DrawOneLine(HDC hDC, LPPOINT lpPointStart, LPPOINT lpPointEnd, COLORREF cr)
{
...
int CDlgMAFPhase6::DrawArrowLine(HDC hDC, LPPOINT lpPointStart, LPPOINT lpPointEnd, 
...
void CDlgMAFPhase6::OnBnClickedBtnMaf6PauseFlash()
{
	if (m_bPause)
	{
		m_bPause = FALSE;
		GetDlgItem(IDC_BTN_MAF6_PAUSE_FLASH)->SetWindowText(_T("暂定"));
		SetTimer(enmTimer_Step1, 500, NULL);
	}
	else
	{
		m_bPause = TRUE;
		GetDlgItem(IDC_BTN_MAF6_PAUSE_FLASH)->SetWindowText(_T("开始"));
		m_bStep1 = FALSE;
		m_bStep2 = FALSE;
		m_bStep3 = FALSE;
		m_bStep4 = FALSE;
